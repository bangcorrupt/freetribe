...
static Aleph_MonoSynth g_synth[POLYSYNTH_NUM_VOICES];

static uint8_t g_voice_index;
...
void module_init(void) {
    ...
    for (i = 0; i < POLYSYNTH_NUM_VOICES; i++) {
        Aleph_MonoSynth_init(&g_synth[i], &g_aleph);
    }
...
void module_process(fract32 *in, fract32 *out) {
    ...
    for (i = 0; i < POLYSYNTH_NUM_VOICES; i++) {
        ...
        //
        output += Aleph_MonoSynth_next(&g_synth[i]);
    }
...
void module_set_param(uint16_t param_index, int32_t value) {
    ...
    case PARAM_FREQ:
        for (i = 0; i < POLYSYNTH_NUM_VOICES; i++) {
            Aleph_MonoSynth_set_freq(&g_synth[i], value);
        }
        break;

    case PARAM_TUNE:
        for (i = 0; i < POLYSYNTH_NUM_VOICES; i++) {
            Aleph_MonoSynth_set_freq_offset(&g_synth[i], value);
        }
        break;
    ...
    case PARAM_GATE:
        Aleph_MonoSynth_set_gate(&g_synth[i], value);
        Aleph_MonoSynth_set_gate(&g_synth[i], value);
        Aleph_MonoSynth_set_gate(&g_synth[i], value);
        break;
    ...
    case PARAM_AMP_ENV_ATTACK:
        for (i = 0; i < POLYSYNTH_NUM_VOICES; i++) {
            Aleph_MonoSynth_set_amp_env_attack(&g_synth[i], value);
        }
        break;

    case PARAM_AMP_ENV_DECAY:
        Aleph_MonoSynth_set_amp_env_decay(&g_synth[i], value);
        break;

    case PARAM_AMP_ENV_SUSTAIN:
        Aleph_MonoSynth_set_amp_env_sustain(&g_synth[i], value);
        break;

    case PARAM_AMP_ENV_RELEASE:
        Aleph_MonoSynth_set_amp_env_release(&g_synth[i], value);
        break;

    case PARAM_AMP_ENV_DEPTH:
        Aleph_MonoSynth_set_amp_env_depth(&g_synth[i], value);
        // Aleph_MonoSynth_set_amp_env_depth(
        //     &g_synth[i], mult_fr1x32x32(g_synth[i]->amp_env_depth,
        //     g_velocity));
        break;
    ...
    case PARAM_FILTER_ENV_ATTACK:
        Aleph_MonoSynth_set_filter_env_attack(&g_synth[i], value);
        break;

    case PARAM_FILTER_ENV_DECAY:
        Aleph_MonoSynth_set_filter_env_decay(&g_synth[i], value);
        break;

    case PARAM_FILTER_ENV_SUSTAIN:
        Aleph_MonoSynth_set_filter_env_sustain(&g_synth[i], value);
        break;

    case PARAM_FILTER_ENV_RELEASE:
        Aleph_MonoSynth_set_filter_env_release(&g_synth[i], value);
        break;

    case PARAM_FILTER_ENV_DEPTH:
        Aleph_MonoSynth_set_filter_env_depth(&g_synth[i], value);
        break;

    case PARAM_PITCH_ENV_ATTACK:
        Aleph_MonoSynth_set_pitch_env_attack(&g_synth[i], value);
        break;

    case PARAM_PITCH_ENV_DECAY:
        Aleph_MonoSynth_set_pitch_env_decay(&g_synth[i], value);
        break;

    case PARAM_PITCH_ENV_SUSTAIN:
        Aleph_MonoSynth_set_pitch_env_sustain(&g_synth[i], value);
        break;

    case PARAM_PITCH_ENV_RELEASE:
        Aleph_MonoSynth_set_pitch_env_release(&g_synth[i], value);
        break;

    case PARAM_PITCH_ENV_DEPTH:
        Aleph_MonoSynth_set_pitch_env_depth(&g_synth[i], value);
        break;

    case PARAM_CUTOFF:
        Aleph_MonoSynth_set_cutoff(&g_synth[i], value);
        break;

    case PARAM_RES:
        Aleph_MonoSynth_set_res(&g_synth[i], value);
        break;

    case PARAM_OSC_TYPE:
        Aleph_MonoSynth_set_shape(&g_synth[i], value);
        break;

    case PARAM_FILTER_TYPE:
        Aleph_MonoSynth_set_filter_type(&g_synth[i], value);
        break;

    case PARAM_AMP_LFO_DEPTH:
        Aleph_MonoSynth_set_amp_lfo_depth(&g_synth[i], value);
        break;
    //
    case PARAM_AMP_LFO_SPEED:
        Aleph_MonoSynth_set_amp_lfo_freq(&g_synth[i], value);
        break;

    case PARAM_FILTER_LFO_DEPTH:
        Aleph_MonoSynth_set_filter_lfo_depth(&g_synth[i], value);
        break;

    case PARAM_FILTER_LFO_SPEED:
        Aleph_MonoSynth_set_filter_lfo_freq(&g_synth[i], value);
        break;

    case PARAM_PITCH_LFO_DEPTH:
        Aleph_MonoSynth_set_pitch_lfo_depth(&g_synth[i], value);
        break;

    case PARAM_PITCH_LFO_SPEED:
        Aleph_MonoSynth_set_pitch_lfo_freq(&g_synth[i], value);
        break;
