...
void filter_2p_lo_set_hz(filter_2p_lo* f, fix16 hz) {
  f32 fc =  (float) exp(-2.0 * M_PI * (double)(fix16_to_float(hz)) * fSrInv );
// / (float)(f->sr) );
...
void filter_2p_hi_init(filter_2p_hi *f) {
    ...
    filter_2p_hi_calc_coeffs(10.f, 1.4142135623730951f, &(f->a), &(f->b),
                             &(f->g));
}
    ...
    f->y = shl_fr1x32(
        sub_fr1x32(
            add_fr1x32(
                mult_fr1x32x32(
                    f->a,
                    add_fr1x32(x, sub_fr1x32(f->x2, shl_fr1x32(f->x1, 1)))),
...
void filter_2p_hi_set_alpha(filter_2p_hi *f, fract32 a) { f->a = a; }

void filter_2p_hi_set_beta(filter_2p_hi *f, fract32 b) { f->b = b; }

void filter_2p_hi_set_gamma(filter_2p_hi *f, fract32 g) { f->g = g; }

// calculate coefficients given cutoff in hz and damping factor
void filter_2p_hi_calc_coeffs(float hz, float d, fract32 *a, fract32 *b,
                              fract32 *g) {
