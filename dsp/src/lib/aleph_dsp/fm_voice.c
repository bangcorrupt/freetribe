...
void fm_voice_next(fm_voice *v) {
    ...
    normalised_logSlew(&(v->noteBaseFreq),
                       fix16_mul_fract(v->noteHz, v->noteTune), v->portamento);
    for (i = 0; i < v->nOps; i++) {
        envNext[i] = trunc_fr1x32(env_adsr_next(&(v->opEnv[i])));
        opFreqTarget = shr_fr1x32(
            fix16_mul_fract(v->noteBaseFreq, v->opTune[i]), FM_OVERSAMPLE_BITS);
    ...
    for (j = 0; j < FM_OVERSAMPLE; j++) {
        ...
        for (i = 0; i < v->nOps; i++) {
            ...
            if (v->opMod2Source[i] < v->nOps) {
                ...
                opMod = add_fr1x16(
                    opMod, multr_fr1x16(modPoints[v->opMod2Source[i] - v->nOps],
                                        v->opMod2Gain[i]));
            }
            ...
            nextOpOutputs[i] = multr_fr1x16(envNext[i], oscSignal);
        }
        ...
        for (i = 0; i < v->nOps; i++) {
            ...
            oversample_outs[i][j] =
                shr_fr1x16(nextOpOutputs[i], FM_OVERSAMPLE_BITS);
        }
    }
    ...
    for (i = 0; i < v->nOps; i++) {
        ...
        for (j = 0; j < FM_OVERSAMPLE; j++) {
            v->opOutputs[i] =
                add_fr1x16(oversample_outs[i][j], v->opOutputs[i]);
        }
    }
}
