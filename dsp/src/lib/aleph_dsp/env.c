...
void env_asr_init(env_asr *env) { env_asr_off(env); }
...
// get the next value
fract32 env_asr_next(env_asr *env) { return (*(env->stateFP))(env); }
...
static fract32 env_next_atk_neg(env_asr *env) {
    ...
    if (env->x == FR32_MAX) {
        ...
        env->y =
            (fract32)(fix16_mul_fract((fix16)(env->y >> 16), env->aUp) << 16);
        env->x = add_fr1x32(env->x, (fract32)env->rUp);
        if (env->x < 0) {
            env->x = FR32_MAX;
        }
    ...
    // interpolate for curve
    return add_fr1x32(mult_fr1x32x32(env->y, cabs),
...
static fract32 env_next_rel_pos(env_asr *env) {
    ...
    // interpolate for curve
    return add_fr1x32(mult_fr1x32x32(env->y, env->cDn),
...
// sustain stage
static fract32 env_next_sus(env_asr *env) { return FR32_MAX; }

// not playing
static fract32 env_next_off(env_asr *env) { return 0; }
