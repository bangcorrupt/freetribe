...
fract32 fade_lin_calc(fadeLin *fade) {
    ...
    ret = add_fr1x32(ret,
                     mult_fr1x32x32(fade->y, sub_fr1x32(FR32_MAX, fade->pos)));
    return ret;
}
...
fract32 fade_ep_calc(fadeEP *fade) {
    ...
    fix16 xIdx =
        fix16_mul_fract(FRACT_FIX16(fade->pos), FADE_SINE_BUF_SIZE_FIX);
    ...
    return add_fr1x32(mult_fr1x32x32(fade->x, xmul),
                      mult_fr1x32x32(fade->y, ymul));
}
