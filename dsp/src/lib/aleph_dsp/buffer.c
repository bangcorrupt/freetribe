...
static fract32 a, b;
  ...
  while(idxB > tap->loop) { idxB -= tap->loop; }
  // we can assume idxA is already wrapped
  tap->buf->data[tap->idx.i] = add_fr1x32(a,
mult_fr1x32x32(tap->buf->data[tap->idx.i], preLevel));
  tap->buf->data[tap->idx.i] = add_fr1x32(b,
mult_fr1x32x32(tap->buf->data[idxB], preLevel)); tap->buf->data[idxB] = b;
}
*/
...
void buffer_tapN_set_inc(bufferTapN *tap, u32 inc) { tap->inc = inc; }
...
/* // write (overwrites old contents) */
/* void buffer_tapN_write_fade(bufferTapN *tapA, bufferTapN *tapB, fract32 val,
 * fract32 pos) {  */
/*   //  tap->buf->data[tap->idx] = val; */
...
/* void buffer_tapN_mix_fade(bufferTapN *tapA, bufferTapN *tapB, fract32 val,
 * fract32 preLevel, fract32 pos) {  */
/*   tap->buf->data[tap->idx] =  */
/*     add_fr1x32(  mult_fr1x32x32(tap->buf->data[tap->idx], preLevel), val );
 */
/* } */
...
/* // interpolated addition of input to buffer contents */
/* void buffer_tapN_add_fade(bufferTapN *tapA, bufferTapN *tapB, fract32 val,
 * fract32 pos) {  */
/*     tap->buf->data[tap->idx] = add_fr1x32(  tap->buf->data[tap->idx], val );
 */
/* } */
...
void bufferTap24_8_set_rate(bufferTap24_8 *tap, s32 inc) { tap->inc = inc; }
...
void bufferTap24_8_set_loop(bufferTap24_8 *tap, s32 loop) { tap->loop = loop; }

void bufferTap24_8_syncN(bufferTap24_8 *tap, bufferTapN *target,
                         s32 offset_subsamples) {
    tap->idx =
        ((256 * target->idx - offset_subsamples) + tap->loop) % tap->loop;
}

void bufferTap24_8_set_pos(bufferTap24_8 *tap, s32 idx) { tap->idx = idx; }
