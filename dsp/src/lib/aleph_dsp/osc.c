...
static inline void osc_calc_wm(osc *osc) {
    ...
    //- with maximal bandlimiting, want to limit shape to a function of unit
    //freq
    ...
    sm = sub_fr1x32(sm,
                    mult_fr1x32x32( (fract32)(fix16_sub(osc->inc, incMin) *
    shapeLimMul), osc->bandLim
                                    )
                    );
    if(sm < 0) { sm = 0; }
    osc->shapeMod = sm;
...
void osc_set_pm(osc *osc, fract32 amt) { filter_1p_lo_in(&(osc->lpPm), amt); }

// shape modulation amount
void osc_set_wm(osc *osc, fract32 amt) { filter_1p_lo_in(&(osc->lpWm), amt); }

// phase modulation input
void osc_pm_in(osc *osc, fract32 val) { osc->pmIn = val; }

// shape modulation input
void osc_wm_in(osc *osc, fract32 val) { osc->wmIn = val; }

// set bandlimiting
void osc_set_bl(osc *osc, fract32 bl) { osc->bandLim = bl; }
